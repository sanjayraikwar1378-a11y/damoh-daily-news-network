...
export function NotificationCenter() {
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      ...
      if (mobileModalRef.current && mobileModalRef.current.contains(target)) return;
      setIsOpen(false);
    };
  ...
  // Lock body scroll on small mobile screens when modal is active
  useEffect(() => {
  ...
  // Reusable notification inner content for both desktop dropdown and mobile sheet
  const renderPanelContent = () => (
    <>
      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/60 shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 active:bg-zinc-700 transition-colors"
      ...
      {/* Body Content - Internal scrolling only */}
      <div className="flex-1 min-h-0 overflow-y-auto divide-y divide-zinc-800/60 scrollbar-thin scrollbar-thumb-zinc-700 overscroll-contain">
        {activeTab === "settings" ? (
  ...
  return (
    <div className="relative inline-block" ref={panelRef}>
      ...
      {/* Desktop Notification Dropdown (sm and above) - Attached to Bell in Header */}
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 w-[410px] max-h-[580px] bg-zinc-900 text-zinc-100 rounded-2xl shadow-2xl border border-zinc-800 flex flex-col z-[70] overflow-hidden"
            >
              {renderPanelContent()}
            </motion.div>
          )}
      ...
      {/* Mobile Floating Overlay Portal (< sm) - Renders directly to body, completely detached from header */}
      {typeof document !== "undefined" && createPortal(
        <AnimatePresence>
          {isOpen && (
            <div className="sm:hidden fixed inset-0 z-[70] flex flex-col justify-end pointer-events-none">
              ...
              <motion.div
                ...
                onClick={() => setIsOpen(false)}
                className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[70] pointer-events-auto"
                aria-hidden="true"
              />

              {/* Floating Mobile Notification Panel */}
              <motion.div
                ref={mobileModalRef}
                ...
                className="fixed z-[75] pointer-events-auto inset-x-3.5 bottom-4 max-w-md mx-auto max-h-[min(72vh,calc(100dvh-125px))] bg-zinc-900 text-zinc-100 rounded-2xl shadow-2xl border border-zinc-800 flex flex-col overflow-hidden"
              >
                {/* Mobile Sheet Grab Handle */}
                <div className="flex justify-center pt-2.5 pb-1 bg-zinc-950/60 shrink-0" aria-hidden="true">
