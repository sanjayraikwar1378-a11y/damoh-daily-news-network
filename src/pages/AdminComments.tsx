      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {filteredComments.map((c, index) => {
            const commentKey = c.id ? `comment-${c.id}` : `comment-idx-${index}-${c.createdAt || ''}`;
            return (
              <div key={commentKey} className="p-4 border rounded-xl bg-zinc-50 dark:bg-zinc-900/50 space-y-2">
